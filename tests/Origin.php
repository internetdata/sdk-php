 ...
 * with a body too large to hold, and a stubbed handler answers neither: with
 * `allow_url_fopen` on, Guzzle routes a `stream => true` request to the
 * StreamHandler rather than to curl, so a canned response exercises none of the
 * code that matters.
 */
final class Origin
{
    ...
    {
        $port = self::freePort();
        $dir = sys_get_temp_dir() . '/internetdata-origin-' . bin2hex(random_bytes(6));
        mkdir($dir);
