...
final class OauthTest extends TestCase
{
    ...
    public function testNoOauthRequestCarriesTheApiKey(): void
    {
        ...
        $device = $oauth->deviceAuthorization('internetdata-cli', ['scope' => 'account.read']);
        $oauth->metadata();
        $oauth->exchangeDeviceCode('internetdata-cli', 'mo_dc_x');
        $oauth->exchangeRefreshToken('internetdata-cli', 'mo_rt_x');
        $oauth->revoke('internetdata-cli', 'mo_rt_x');
        $oauth->pollDeviceToken('internetdata-cli', $device);
    ...
    public function testEachOperationRequestsItsEndpointWithExactlyItsFormFields(): void
    {
        ...
        $stub = new OauthStub([['status' => 200, 'body' => self::EVERY_REQUIRED_MEMBER]]);
        self::client($stub)->oauth->deviceAuthorization('internetdata-cli', ['scope' => '', 'resource' => '']);
        $fields = OauthStub::formFields($stub->requests[0]['body']);
        self::assertSame(['client_id' => 'internetdata-cli'], $fields, 'an empty optional field is left out');
    }
    ...
    public function testA2xxDecodesOnPresenceSoAbsentStaysAbsentAndAnEmptyScopePresent(): void
    {
        ...
        foreach ($operations as $section => $operation) {
            foreach (self::$corpus['responses'][$section] as $case) {
                ...
                $args = ['clientId' => 'internetdata-cli', 'deviceCode' => 'mo_dc_x'];
        ...
        foreach (self::$corpus['responses']['revoke'] as $case) {
            $stub = new OauthStub([$case]);
            self::client($stub)->oauth->revoke('internetdata-cli', 'mo_rt_x');
    ...
    public function testAnOptionOauthCannotUseIsRefusedBeforeAnyRequest(): void
    {
        ...
        $calls = [
            ...
            'a scope that is not a string' => static fn (): mixed
                => $oauth->deviceAuthorization('internetdata-cli', ['scope' => ['account.read']]),
            'a negative timeout' => static fn (): mixed
                => $oauth->revoke('internetdata-cli', 'mo_rt_x', ['timeout' => -1]),
        ];
    ...
    private static function timedCalls(): array
    {
        ...
        return [
            ...
            'deviceAuthorization' => static fn (Client $c, array $o): mixed
                => $c->oauth->deviceAuthorization('internetdata-cli', $o),
            'exchangeDeviceCode' => static fn (Client $c, array $o): mixed
                => $c->oauth->exchangeDeviceCode('internetdata-cli', 'mo_dc_x', $o),
            'exchangeRefreshToken' => static fn (Client $c, array $o): mixed
                => $c->oauth->exchangeRefreshToken('internetdata-cli', 'mo_rt_x', $o),
            'revoke' => static fn (Client $c, array $o): mixed
                => $c->oauth->revoke('internetdata-cli', 'mo_rt_x', $o),
            'pollDeviceToken' => static function (Client $c, array $o) use ($device): mixed {
                (new OauthStub([]))->installClock($c->oauth);
                return $c->oauth->pollDeviceToken('internetdata-cli', $device, $o);
            },
        ];
    }
    ...
    {
        return self::settle(
            static fn (): mixed => self::client($stub)->oauth->exchangeDeviceCode('internetdata-cli', 'mo_dc_x'),
        );
    }
    ...
    {
        self::assertInstanceOf(InternetDataException::class, $outcome, "{$label}: settled with a value");
        $class = $outcome::class;
        match ($want['type']) {
            ...
            'client' => self::assertSame(InternetDataException::class, $class, $label),
        };
