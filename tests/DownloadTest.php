...
/**
 * The download path, against a real HTTP origin rather than a stub: what these
 * methods are for is the 302 and a body too large to hold, and neither question
 * survives a canned response.
 */
final class DownloadTest extends TestCase
{
    ...
    {
        $this->tmp = sys_get_temp_dir() . '/internetdata-dl-' . bin2hex(random_bytes(6));
        mkdir($this->tmp);
    }
    ...
    public function testDownloadUrlHandsBackTheLinkAndFetchesNothing(): void
    {
        ...
        self::assertSame(
            ...
            array_column($this->origin->requests(), 'path'),
            'the redirect was chased, so the bytes moved without being asked for',
        );
    }
    ...
    public function testTheKeyReachesTheApiAndNeverObjectStorage(): void
    {
        ...
        self::assertSame('Bearer secret-key', $api['headers']['authorization']);
    ...
    // reads it as a whole database. PHP sees a dropped transfer as a plain EOF,
    // so this only fails if the declared length is checked against what arrived.
    ...
    // The other side of that rule: a refusal that arrives before any byte of the
    // body is still worth repeating, and the file must hold one copy.
    public function testARefusalDuringTheHeaderPhaseIsRetriedWithoutDoublingTheFile(): void
    {
